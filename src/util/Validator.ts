...
export const ConfigSchema = z.object({
    ...
    searchSettings: z
        ...
        // passthrough keeps human-likeness knobs (mode, diversityBase,
        // useGeoLocaleQueries, pointsPerSearch) defined in config.json.
        .passthrough(),
    experimental: z
        .object({
...
// passthrough keeps extra (user/LLM) keys such as openRouterApiKey,
// openRouterReasoningEnabled, openRouterProvider, openRouterUserId, etc., so the
// AI/LLM query-generation settings can live in config.json instead of being
...
const AccountProxySchema = z
    ...
    .superRefine((proxy, ctx) => {
        ...
        if (!hasUrl) {
            if (proxy.proxyAxios || proxy.port !== 0 || hasUsername || hasPassword) {
                ctx.addIssue({
                    code: 'custom',
                    path: ['url'],
...
const defaultConfig: Config = {
    ...
    legacyFallback: { enabled: true, minDelay: '1hr', maxDelay: '2hr', alternate: true },
    workers: {
        doDailySet: true,
