...
export default class ReactFunc {
    ...
    public extractActionIds(jsText: string): {
        ...
        try {
            // Support for both (createServerReference)(id) / createServerReference(id) and the
            // newer registerServerReference(id) wrapper. The argument search window is expanded
            // to 800 chars for Turbopack bundles so the action name is found even when it sits
            // further from the hex id.
